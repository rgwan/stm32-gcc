...
void Set_System(void)
{
  ...
  GPIO_Init(USB_DISCONNECT, &GPIO_InitStructure); //PD9 设置PD9为USB控制引脚 P30  改为PA8
#endif /* USB_USE_EXTERNAL_PULLUP */  
...
#else
  /* Select USBCLK source */
  RCC_USBCLKConfig(RCC_USBCLKSource_PLLCLK_1Div5);//设置USB时钟
...
void Leave_LowPowerMode(void)
{
  ...
  {
    /* Device configured */
    bDeviceState = CONFIGURED;//说明配置成功
     dbg("Leave_LowPowerMode CONFIGURED\r\n");
  }
  else
  {
...
#elif defined(STM32F10X_CL) 
  NVIC_InitStructure.NVIC_IRQChannel = OTG_FS_IRQn;//使能OTG中断
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;
...
#else
  NVIC_InitStructure.NVIC_IRQChannel = USB_LP_CAN1_RX0_IRQn;//接收中断
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//设置中断优先级
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;//设置中断优先级
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//使能中断
  NVIC_Init(&NVIC_InitStructure);

  NVIC_InitStructure.NVIC_IRQChannel = USB_HP_CAN1_TX_IRQn;//发送中断
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//设置中断优先级
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;//设置中断优先级
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//使能中断
  NVIC_Init(&NVIC_InitStructure);
#endif /* STM32F10X_CL */
...
#if defined(STM32F10X_HD) || defined(STM32F10X_XL)  
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;//使能SDIO中断
...
void Get_SerialNum(void)
{
...
#ifdef STM32L1XX_MD//根据芯片类型修改序列号
  ...
  if (Device_Serial0 != 0)
  {
    IntToUnicode (Device_Serial0, &MASS_StringSerial[2] , 8);//存储序列号
