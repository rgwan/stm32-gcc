...
void MASS_init()
{
  ...
  #ifdef  USBPROP_DEBUG 
      dbg("MASS_init Get_SerialNum \r\n");
     #endif
  pInformation->Current_Configuration = 0;

  /* Connect the device */
  PowerOn();//使能usb模拟部分，并置复位请求，挂起请求，及唤醒请求使能  USB_CNTR
 
  /* Perform basic device initialization operations */
  USB_SIL_Init();//正确传输使能，复位使能 P412 USB_CNTR
  
   #ifdef  USBPROP_DEBUG 
      dbg("MASS_init power on\r\n");
     #endif
      
  bDeviceState = UNCONNECTED;//设置未连接标志
}
...
void MASS_Reset()
{
  ...
  SetBTABLE(BTABLE_ADDRESS);//BTABLE USB分组缓冲区描述表地址寄存器设置为0 P416

  /* Initialize Endpoint 0 */
  SetEPType(ENDP0, EP_CONTROL);//EP_TYPE 设置端点类型 P418  EP_CONTROL控制端点
  SetEPTxStatus(ENDP0, EP_TX_NAK);//STAT_TX 发送编码 ，P418  端点以NAK分组响应所有的发送请求
  SetEPRxAddr(ENDP0, ENDP0_RXADDR);//设置接收地址 P419  0x18？？？
  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);//USB_COUNTn_RX 接收字节数设置 P420
  SetEPTxAddr(ENDP0, ENDP0_TXADDR);//设置发送地址 P419  =0x58？？？
  Clear_Status_Out(ENDP0);//EP_KIND P410  批量端点
  SetEPRxValid(ENDP0);//设置STAT_RX，有效 P417

  /* Initialize Endpoint 1 */
  SetEPType(ENDP1, EP_BULK);
  SetEPTxAddr(ENDP1, ENDP1_TXADDR);//0x98
  SetEPTxStatus(ENDP1, EP_TX_NAK);
  SetEPRxStatus(ENDP1, EP_RX_DIS);//STAT_RX P417 数据接收状态

  /* Initialize Endpoint 2 */
  SetEPType(ENDP2, EP_BULK);
  SetEPRxAddr(ENDP2, ENDP2_RXADDR);//0xd8
  SetEPRxCount(ENDP2, Device_Property.MaxPacketSize);
  SetEPRxStatus(ENDP2, EP_RX_VALID);
  SetEPTxStatus(ENDP2, EP_TX_DIS);


  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);//为何重新再设置包长???
  SetEPRxValid(ENDP0);//为何再重新设置有效?????

  /* Set the device to response on default address */
  SetDeviceAddress(0);//设置芯片地址
#endif /* STM32F10X_CL */

  bDeviceState = ATTACHED;//设置设备标志

  CBW.dSignature = BOT_CBW_SIGNATURE;//与批量传输有关
  Bot_State = BOT_IDLE;

  USB_NotConfigured_LED();//关闭指示灯
}
