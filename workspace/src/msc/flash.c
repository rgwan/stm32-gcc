...
#define FILEDATA_SECTOR_COUNT	2
...
int ramdisk_init(void)
{
	...
	//}
	memcpy(ramdata, BootSector, sizeof(BootSector));
	ramdata[SECTOR_SIZE - 2] = 0x55;
	ramdata[SECTOR_SIZE - 1] = 0xaa;
	
	memcpy(ramdata + (SECTOR_SIZE * 1), FatSector, sizeof(FatSector));

	memcpy(ramdata + (SECTOR_SIZE * 2), FatSector, sizeof(FatSector));
	
	memcpy(ramdata + (SECTOR_SIZE * 3), DirSector, sizeof(DirSector));
	ramdata[SECTOR_SIZE * 3 + 13] = chk;
	// fill ramdata
	
	const uint8_t text[] = "USB Mass Storage Class example. ";
	i = 4 * SECTOR_SIZE;
	dbg("File_Start offset= 0x%x", (unsigned int)i);
...
int ramdisk_read(uint8_t *copy_to,uint32_t offset,
	const uint32_t length)
{
	memset(copy_to, 0, length);
	dbg("read: offset = 0x%x, length = 0x%x\r\n",(unsigned int) offset, (unsigned int) length);
	if (offset < (SECTOR_SIZE * SECTOR_COUNT)) {
		memcpy(copy_to, ramdata + offset,
		 length);
	}
	return Blk_OK;
}

int ramdisk_write(const uint8_t *copy_from, 
	const uint32_t offset, const uint32_t length)
{
	(void)copy_from;
	// ignore writes
	dbg("write: offset = 0x%x, length = 0x%x\r\n",(unsigned int) offset, (unsigned int) length);
	memcpy(ramdata + offset, copy_from, length);
	return Blk_OK;
}
