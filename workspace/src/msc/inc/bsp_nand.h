...
*/
#define NAND_MAKER_ID	0xC8
#define NAND_DEVICE_ID	0xF1
#define NAND_THIRD_ID 	0x00
#define NAND_FOURTH_ID	0x00
...
/* 用于HY27UF081G2A    K9F1G08 */
	...
	#define NAND_BLOCK_COUNT			1000 /* 块个数 */
	#define NAND_PAGE_TOTAL_SIZE		(NAND_PAGE_SIZE + NAND_SPARE_AREA_SIZE)	/* 页面总大小 */
	
	
#elif NAND_TYPE == GD5F1GQ4
	#define NAND_PAGE_SIZE             ((uint16_t)0x0800) /* 2 * 1024 bytes per page w/o Spare Area */
	#define NAND_BLOCK_SIZE            ((uint16_t)0x0040) /* 64 pages per block */
	#define NAND_ZONE_SIZE             ((uint16_t)1020) /* 1024 Block per zone */
	#define NAND_SPARE_AREA_SIZE       ((uint16_t)0x0040) /* last 64 bytes as spare area */
	#define NAND_MAX_ZONE              ((uint16_t)0x0001) /* 1 zones of 1024 block */
	#define MAX_PHY_BLOCKS_PER_ZONE  1020	/* 每个区最大物理块号 */
	#define MAX_LOG_BLOCKS_PER_ZONE  1000	/* 每个区最大逻辑块号 */
	
	#define NAND_BLOCK_COUNT			1020 /* 块个数 */
	#define NAND_PAGE_TOTAL_SIZE		(NAND_PAGE_SIZE + NAND_SPARE_AREA_SIZE)	/* 页面总大小 */
...
*/		
#define DATA_BLOCK_PERCENT		95 	/* 数据块占总有效块数的百分比 */
